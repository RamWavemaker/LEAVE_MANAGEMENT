...
//     Select buttons and modal elements
    ...
    const applyleavesbutton = document.querySelector('.applyleavesbutton');
     ...
     const holidaysbutton = document.querySelector('.holidaysbutton');
     var Usergender = null;
...
//            dashboard.appendChild(row);
    ...
    function handleGenderBasedOptions() {
            ...
            console.log('Usergender:', Usergender); // Debuging Usergender
            console.log('LeaveTypeSelect options before:', leaveTypeSelect.innerHTML); // Debuging options before removal
            ...
            console.log('LeaveTypeSelect options after:', leaveTypeSelect.innerHTML); // Debuging options after removal
        }
        ...
        function removeOption(selectElement, valueToRemove) {
            ...
            if (option) {
                console.log('Removing option:', option.text); // Debuging removed option
    ...
    leaverequestform.addEventListener('submit',(e) =>{
        ...
        if (leaveLimits[leavetypeinput] <= 0) {
             openMessageModal(`There are no ${leavetypeinput} Leaves Left`);
             return;
        }
         ...
         if (fromDate < today) {
               openMessageModal("The 'From Date' cannot be a past date.");
               return;
         }

         if(toDate < fromDate){
               openMessageModal("The 'To Date' cannot Before 'From Date'");
               return;
         }

         if (appliedDate.getTime() !== today.getTime()) {
              openMessageModal("The 'Applied Date' should be today's date.");
              return;
         }

         const diffTime = Math.abs(toDate - fromDate);
         const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)) + 1; // we should Include both start and end dates so +1

         if (diffDays > leaveLimits[leavetypeinput]) {
               openMessageModal(`The number of leave days (${diffDays}) exceeds the remaining ${leavetypeinput} leave days.`)
             return;
         }
        // Gender-based validation
        if (leavetypeinput === 'PATERNITY' && Usergender === 'FEMALE') {
            openMessageModal("PATERNITY LEAVES ARE NOT APPLIED FOR FEMALE");
            return;
        }

        if (leavetypeinput === 'MATERNITY' && Usergender === 'MALE') {
            openMessageModal("MATERNITY LEAVES ARE NOT APPLIED FOR MALE");
            return;
        }
    ...
    function givingNameforManager() {
        ...
        .then(data => {
            ...
            const flattenedData = data.flat(); //the response is nested array so we flattend
            ...
            flattenedData.forEach(employee => {
                ...
                option.value = employee.Id; // UsIng 'Id' field from data
                option.textContent = employee.name; // Using 'name' field from data
                select.appendChild(option);
            });

            select.addEventListener('change', giveLeavesOfEmployee);
        })
        .catch(error => {
    ...
    function giveLeavesOfEmployee(event) {
        ...
        .then(data => {
            ...
            if (!Array.isArray(data) || data.length === 0) {
                 ...
                 givingNameforManager();  //giving names for manager
            ...
            data.forEach(leave => {
                ...
                leavediv.innerHTML = `
                    <div class='card-body'>
                        ...
                        <p class='card-text'>Leave Comment: ${leave.leaveComment}</p>
                        <p class='card-text'>Status : ${leave.status}</p>
                        <p id='leaveidhide' class='card-text'>Leave Id: ${leave.Id}</p>
                ...
                `;
//                document.getElementById('leaveidhide').style.display = 'none';
                col.appendChild(leavediv);
                row.appendChild(col);
            });
            dashboard.appendChild(row);
...
//               dashboard.appendChild(row);
   ...
   async function fetchEmployeesLeaveData() {
       try {
           ...
           console.error('Error fetching leave data:', error);
           openMessageModal('An error occurred while fetching leave data. Please try again later.');
       }
   }
   ...
   function displayEmployeesLeaveData(leaveData) {
       ...
       if (typeof leaveData !== 'object' || leaveData === null) {
           openMessageModal("leaveData is Empty");
           return;
       }
       ...
       // Iterating through each employee's leave data
   ...
   function displayHolidays(){
       fetch("/LeaveManagement/holidaysservlet",{
           method: 'GET',
           headers: {
               'Content-Type': 'application/json',
           },
       ...
       .then(data =>{
           ...
           const headers = ['ID', 'Holiday Date', 'Occasion'];
